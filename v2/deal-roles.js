...
(() => {
  ...
  function showRole(key,open=false){const d=descriptions[key];section.querySelectorAll('[data-edu-role]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.eduRole===key)));q('#eduRoleExplanation').innerHTML='<details'+(open?' open':'')+'><summary>'+d.title+'</summary><div class="edu-role-pair"><div><b>ما لهم؟</b><p>'+d.rights+'</p></div><div><b>ما عليهم؟</b><p>'+d.duties+'</p></div></div><p class="edu-role-place"><b>موقعهم في الهيكل</b> '+d.place+'</p></details>';}
  const compGuidance={
  ...
  section.addEventListener('click',event=>{const button=event.target.closest('[data-edu-role]');if(button)showRole(button.dataset.eduRole,true);});
